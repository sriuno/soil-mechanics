...
# 지점별 토압 연산
p_tot_0 = max(0.0, q * Ka - 2 * c * math.sqrt(Ka))
p_tot_zw = max(0.0, (q + gamma * zw) * Ka - 2 * c * math.sqrt(Ka))

gamma_sub = gamma_sat - gamma_w
sigma_v_eff_H = q + gamma * zw + gamma_sub * (H - zw)
p_soil_H = max(0.0, sigma_v_eff_H * Ka - 2 * c * math.sqrt(Ka))
u_H = gamma_w * (H - zw) if H > zw else 0.0
p_tot_H = p_soil_H + u_H

# 합력 및 작용점 계산
P1 = ((p_tot_0 + p_tot_zw) / 2.0) * zw
y1 = (H - zw) + (zw / 3.0) * ((2 * p_tot_0 + p_tot_zw) / (p_tot_0 + p_tot_zw)) if (p_tot_0 + p_tot_zw) > 0 else (H - zw) + zw/2.0

P2 = ((p_tot_zw + p_tot_H) / 2.0) * (H - zw)
y2 = ((H - zw) / 3.0) * ((2 * p_tot_zw + p_tot_H) / (p_tot_zw + p_tot_H)) if (p_tot_zw + p_tot_H) > 0 else (H - zw)/2.0
...
with col1:
    ...
    df_res = pd.DataFrame({
        ...
        "총 수평토압 (kN/m²)": [f"{p_tot_0:.2f}", f"{p_tot_zw:.2f}", f"{p_tot_H:.2f}"],
...
with col2:
    ...
    z_layers = [0, zw, H]
    p_layers = [p_tot_0, p_tot_zw, p_tot_H]
